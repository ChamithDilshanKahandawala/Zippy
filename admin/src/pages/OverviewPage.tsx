...
import { Users, Car, MapPinned, DollarSign } from 'lucide-react';
...
export const OverviewPage: React.FC = () => {
  ...
  const todayEarnings = earnings.reduce((sum, e) => sum + e.amount, 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <StatsCard label="Total Riders" value={totalRiders} icon={Users} />
        <StatsCard label="Active Rides" value={activeRides} icon={MapPinned} />
        <StatsCard label="Weekly Earnings" value={`LKR ${todayEarnings.toLocaleString()}`} icon={DollarSign} />
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-4 shadow-sm transition-colors duration-300">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-slate-400 dark:text-slate-500">Realtime Firestore data can plug into this chart later.</span>
        </div>
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={earnings} margin={{ left: -20, right: 10 }}>
              ...
              <Tooltip cursor={{ stroke: '#1e293b', strokeWidth: 1 }} contentStyle={{ backgroundColor: '#020617', border: '1px solid #1e293b', borderRadius: 8 }} />
